...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ddifunc="ddi:functions"
    ...
    <xsl:function name="ddifunc:rename" as="xs:string?">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$type = 'class'">
                <xsl:value-of
                    select="ddifunc:upper-case-first(ddifunc:replaceUnderscore(ddifunc:camelize($name)))"
                />
            </xsl:when>
            <!-- property -->
            <xsl:when test="$type = 'property'">
                <xsl:value-of
                    select="ddifunc:lower-case-first(ddifunc:replaceUnderscore(ddifunc:camelize($name)))"
                />
            </xsl:when>
        </xsl:choose>
    </xsl:function>

    <xsl:function name="ddifunc:replaceColon" as="xs:string?">
        <xsl:param name="arg" as="xs:string?"/>
    ...
    <xsl:function name="ddifunc:cleanName" as="xs:string">
        <xsl:param name="arg" as="xs:string?"/>

        <xsl:value-of
            select="ddifunc:upper-case-first(ddifunc:replaceUnderscore(ddifunc:replaceColon(ddifunc:camelize($arg))))"
        />
    </xsl:function>
</xsl:stylesheet>
